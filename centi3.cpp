...
int main()
{
	...
	for (int i = 0; i < totalcentipieces; i++) 
	{
        ...
        // Inside the main loop, after the centipede movement logic
//for (int i = 0; i < totalcentipieces; i++) {
    // ... (centipede movement logic)

    // Check for collision with bullet
    if (bullet[exists] && bullet[x] < centipedeX[i] + boxPixelsX &&
        bullet[x] + boxPixelsX > centipedeX[i] &&
        bullet[y] < centipedeY[i] + boxPixelsY &&
        bullet[y] + boxPixelsY > centipedeY[i]) {

        // Split the centipede at the hit segment
        // You may want to adjust these indices based on your centipede's structure
        const int splitIndex = i;
        const int remainingSegments = totalcentipieces - splitIndex;

        // Move the hit segment and the segments after it to opposite directions
        for (int j = splitIndex + 1; j < totalcentipieces; j++) {
            centipedeX[j] += 2.0;  // Move the remaining segments to the right
        }

        // Set the bullet to inactive
        bullet[exists] = false;

        // Break out of the loop to avoid further splitting on the same frame
        break;
    }
}
